...
export default function LandingPage() {
  ...
  >
                    Sign Out
                  </button><Button asChild size="sm"><Link to="/dashboard">
                      Go to App <ArrowRight className="h-3.5 w-3.5" /></Link></Button></> : <><Link
    to="/login"
  ...
  >{user ? <Button asChild size="lg"><Link to="/dashboard">
                    Go to Dashboard
  ...
  ><span className="text-3xl sm:text-4xl font-black text-orange-500 mb-1">{s.value}</span><span className="text-xs text-gray-400 font-semibold uppercase tracking-wider">{s.label}</span></div>)}</motion.div><motion.div
    ...
    className="mt-24"
  ><div className="text-center mb-12"><span className="inline-block px-3 py-1 rounded-full bg-yellow-50 border border-yellow-200 text-yellow-600 text-xs font-bold uppercase tracking-wider mb-4">
                Features
              </span><h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-3">
                Everything you need
              </h2><p className="text-sm text-gray-400 max-w-lg mx-auto">
                From reservation to checkout, SPMS handles the full parking lifecycle
                with enterprise-grade reliability.
              </p></div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">{features.map((f, i) => <motion.div
    key={i}
