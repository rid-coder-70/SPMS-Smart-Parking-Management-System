...
import type { User, Page } from '@/common/types';
    ...
    <div className="space-y-6 animate-fade-in max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2.5">
            <Users className="h-6 w-6 text-orange-500" /> User Management
          </h1>
          <p className="text-sm text-gray-500 mt-1">Manage platform accounts, roles, and security credentials.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-gray-400">Loading accounts...</td>
                </tr>
                ...
                <tr key={u.id} className="hover:bg-orange-50/30 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    {u.vehicleType ? (
                      <span className="badge bg-gray-100 text-gray-700 font-mono text-[10px]">
                        {u.vehicleType} • {u.vehicleNumber || 'No Plate'}
                      </span>
                    ) : (
                      <span className="text-gray-400 font-italic">None</span>
                    )}
                  </td>
                  <td className="px-4 py-3.5">
                    <span className={`badge ${u.role === 'ADMIN' ? 'bg-violet-50 text-violet-700 border-violet-200' : 'bg-gray-50 text-gray-600'}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="px-4 py-3.5">
                    <span className={`badge ${u.accountStatus === 'ACTIVE' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-600 border-red-200'}`}>
                      {u.accountStatus}
                    </span>
                  </td>
      ...
      {resetModalUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <form onSubmit={handleResetPassword} className="bg-white rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="flex gap-2 pt-2">
              <button type="button" onClick={() => setResetModalUser(null)} className="btn-secondary flex-1">Cancel</button>
