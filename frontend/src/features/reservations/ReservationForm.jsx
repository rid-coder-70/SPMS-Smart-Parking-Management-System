...
const ReservationForm = ({ onSuccess }) => {
  ...
  return <div className="space-y-8">{success && <div className="alert-success animate-fade-in"><CheckCircle className="h-5 w-5 text-green-500 shrink-0 mt-0.5" /><div><p className="font-semibold text-green-700">Reservation Created!</p><p className="text-xs mt-0.5 text-green-600">
    ...
    className="w-full"
  /></div>{lotId && <div className="card"><div className="flex items-center gap-3 mb-4"><div className="w-8 h-8 rounded-full bg-orange-100 border border-orange-200 flex items-center justify-center text-orange-600 font-bold text-sm">2</div><h3 className="font-semibold text-gray-900">Choose a Slot</h3>{selectedSlot && <span className="ml-auto badge bg-orange-50 border-orange-200 text-orange-600">
                  ✓ Slot {selectedSlot.slotNumber} selected
                </span>}</div><p className="text-xs text-gray-500 mb-3">Click an <span className="text-green-600 font-medium">AVAILABLE</span> slot to select it.</p><SlotGrid
    lotId={lotId}
    selectable
    onSlotClick={handleSlotClick}
  /></div>}{selectedSlot && <div className="card"><div className="flex items-center gap-3 mb-6"><div className="w-8 h-8 rounded-full bg-orange-100 border border-orange-200 flex items-center justify-center text-orange-600 font-bold text-sm">3</div><h3 className="font-semibold text-gray-900">Set Date, Time & Duration</h3></div><div className="grid sm:grid-cols-3 gap-4"><div><label className="label flex items-center gap-1.5"><CalendarDays className="h-4 w-4 text-orange-500" /> Date
                </label><input
    id="reservation-date"
    type="date"
    className="input"
    ...
    required
  /></div><div><label className="label flex items-center gap-1.5"><Clock className="h-4 w-4 text-orange-500" /> Start Time
                </label><input
    ...
    required
  /></div><div><label className="label flex items-center gap-1.5"><Timer className="h-4 w-4 text-orange-500" /> Duration
                </label><select
    id="reservation-duration"
    className="input"
    value={durationMinutes}
    onChange={(e) => setDurationMinutes(Number(e.target.value))}
  >{DURATION_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div></div><button
    id="reservation-submit"
    type="submit"
    disabled={submitting}
    className="btn-primary w-full mt-6"
  >{submitting ? <span className="flex items-center gap-2"><span className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  Booking...
                </span> : "Confirm Reservation"}</button></div>}</form></div>;
};
